#!/bin/bash
# Run all PersonalLLM experiments: BT, MeanVar, and anchor variants
...
# Usage: bash scripts/run_anchor_personalllm_split.sh

set -e

export WANDB_PROJECT="huggingface_personalllm"
...
echo "============================================"
echo "  PersonalLLM Experiment Suite (BT / MV / Anchor)"
...
#         --sample_map data/personalllm_sample_map.json
# done
...
for MODEL_NAME in "${MODEL_NAMES[@]}"; do
    ...
    for SEED in "${SEEDS[@]}"; do
        ...
        for LEARNING_RATE in "${LEARNING_RATES[@]}"; do
            ...
            # echo "=== Train: BT on PersonalLLM ==="
            # CUDA_VISIBLE_DEVICES=${CUDA_DEVICES} accelerate launch --config_file configs/accelerate_config.yaml \
            #     --num_processes ${NUM_GPUS} \
            #     -m src.train --config configs/bt_personalllm.yaml \
            #     --model_name ${MODEL_NAME} \
            #     --learning_rate ${LEARNING_RATE} \
            #     --seed ${SEED}
            # EVAL_QUEUE+=("python -m src.evaluate \
            #     --config configs/bt_personalllm.yaml \
            #     --checkpoint $(ckpt_dir bt_personalllm ${MODEL_SHORT} ${SEED} ${LR_KEY})")

            # # --- MeanVar (no anchor dependency, run once per LR/seed) ---
            # echo "=== Train: MV on PersonalLLM ==="
            # CUDA_VISIBLE_DEVICES=${CUDA_DEVICES} accelerate launch --config_file configs/accelerate_config.yaml \
            #     --num_processes ${NUM_GPUS} \
            #     -m src.train --config configs/mv_personalllm.yaml \
            #     --model_name ${MODEL_NAME} \
            #     --learning_rate ${LEARNING_RATE} \
            #     --seed ${SEED}
            # EVAL_QUEUE+=("python -m src.evaluate \
            #     --config configs/mv_personalllm.yaml \
            #     --checkpoint $(ckpt_dir mv_personalllm ${MODEL_SHORT} ${SEED} ${LR_KEY})")
            ...
            for ANCHOR_MODEL_NAME in "${ANCHOR_MODEL_NAMES[@]}"; do
                ...
                for ANCHOR_LAMBDA in "${ANCHOR_LAMBDAS[@]}"; do
                    ...
                    # # MV + 1anchor
                    # echo "=== Train: MV+1anchor on PersonalLLM ==="
                    # CUDA_VISIBLE_DEVICES=${CUDA_DEVICES} accelerate launch --config_file configs/accelerate_config.yaml \
                    #     --num_processes ${NUM_GPUS} \
                    #     -m src.train --config configs/mv_1anchor_personalllm.yaml \
                    #     --model_name ${MODEL_NAME} \
                    ...
                    #     --seed ${SEED}
                    # EVAL_QUEUE+=("python -m src.evaluate \
                    #     --config configs/mv_1anchor_personalllm.yaml \
                    #     --checkpoint $(ckpt_dir mv_1anchor_personalllm ${MODEL_SHORT} ${SEED} ${LR_KEY} ${ANCHOR_MODEL_KEY} ${ANCHOR_LAMBDA})")

                    # MV + 2anchor
                    echo "=== Train: MV+2anchor on PersonalLLM ==="
                    CUDA_VISIBLE_DEVICES=${CUDA_DEVICES} accelerate launch --config_file configs/accelerate_config.yaml \
                        --num_processes ${NUM_GPUS} \
                        -m src.train --config configs/mv_2anchor_personalllm.yaml \
                        --model_name ${MODEL_NAME} \
                        --anchor_model ${ANCHOR_MODEL_KEY} \
                        --anchor_lambda ${ANCHOR_LAMBDA} \
                        --learning_rate ${LEARNING_RATE} \
                        --seed ${SEED}
                    EVAL_QUEUE+=("python -m src.evaluate \
                        --config configs/mv_2anchor_personalllm.yaml \
                        --checkpoint $(ckpt_dir mv_2anchor_personalllm ${MODEL_SHORT} ${SEED} ${LR_KEY} ${ANCHOR_MODEL_KEY} ${ANCHOR_LAMBDA})")
...
echo ""
echo "============================================"
echo "  All PersonalLLM experiments complete!"
