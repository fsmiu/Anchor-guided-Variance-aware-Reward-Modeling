#!/bin/bash
# Run all MultiPref experiments: BT, MeanVar, and anchor variants
# Phase 1: all training runs (serial, full GPU)
# Phase 2: all evaluations (parallel, 1 task per GPU, batches of NUM_GPUS)
# Hardware: 4x NVIDIA A800 80GB PCIe
# Usage: bash scripts/run_anchor_multipref.sh

set -e

export WANDB_PROJECT="huggingface_multipref"
...
echo "============================================"
echo "  MultiPref Experiment Suite (BT / MV / Anchor)"
...
#         --train_data data/multipref_train.json \
#         --val_data data/multipref_heldout.json \
#         --test_data data/multipref_heldout.json
# done
...
#         --dataset_name multipref \
#         --sample_map data/multipref_sample_map.json
# done
...
for MODEL_NAME in "${MODEL_NAMES[@]}"; do
    ...
    for SEED in "${SEEDS[@]}"; do
        ...
        for LEARNING_RATE in "${LEARNING_RATES[@]}"; do
            ...
            # echo "=== Train: BT on MultiPref ==="
            # CUDA_VISIBLE_DEVICES=${CUDA_DEVICES} accelerate launch --config_file configs/accelerate_config.yaml \
            #     --num_processes ${NUM_GPUS} \
            #     -m src.train --config configs/bt_multipref.yaml \
            #     --model_name ${MODEL_NAME} \
            #     --learning_rate ${LEARNING_RATE} \
            #     --seed ${SEED}
            # EVAL_QUEUE+=("python -m src.evaluate \
            #     --config configs/bt_multipref.yaml \
            #     --checkpoint $(ckpt_dir bt_multipref ${MODEL_SHORT} ${SEED} ${LR_KEY})")

            # # --- MeanVar (no anchor dependency, run once per LR/seed) ---
            # echo "=== Train: MV on MultiPref ==="
            # CUDA_VISIBLE_DEVICES=${CUDA_DEVICES} accelerate launch --config_file configs/accelerate_config.yaml \
            #     --num_processes ${NUM_GPUS} \
            #     -m src.train --config configs/mv_multipref.yaml \
            #     --model_name ${MODEL_NAME} \
            #     --learning_rate ${LEARNING_RATE} \
            #     --seed ${SEED}
            # EVAL_QUEUE+=("python -m src.evaluate \
            #     --config configs/mv_multipref.yaml \
            #     --checkpoint $(ckpt_dir mv_multipref ${MODEL_SHORT} ${SEED} ${LR_KEY})")
            ...
            for ANCHOR_MODEL_NAME in "${ANCHOR_MODEL_NAMES[@]}"; do
                ...
                for ANCHOR_LAMBDA in "${ANCHOR_LAMBDAS[@]}"; do
                    ...
                    # # MV + 1anchor
                    # echo "=== Train: MV+1anchor on MultiPref ==="
                    # CUDA_VISIBLE_DEVICES=${CUDA_DEVICES} accelerate launch --config_file configs/accelerate_config.yaml \
                    #     --num_processes ${NUM_GPUS} \
                    #     -m src.train --config configs/mv_1anchor_multipref.yaml \
                    #     --model_name ${MODEL_NAME} \
                    ...
                    #     --seed ${SEED}
                    # EVAL_QUEUE+=("python -m src.evaluate \
                    #     --config configs/mv_1anchor_multipref.yaml \
                    #     --checkpoint $(ckpt_dir mv_1anchor_multipref ${MODEL_SHORT} ${SEED} ${LR_KEY} ${ANCHOR_MODEL_KEY} ${ANCHOR_LAMBDA})")

                    # MV + 2anchor
                    echo "=== Train: MV+2anchor on MultiPref ==="
                    # CUDA_VISIBLE_DEVICES=${CUDA_DEVICES} accelerate launch --config_file configs/accelerate_config.yaml \
                    #     --num_processes ${NUM_GPUS} \
                    #     -m src.train --config configs/mv_2anchor_multipref.yaml \
                    #     --model_name ${MODEL_NAME} \
                    #     --anchor_model ${ANCHOR_MODEL_KEY} \
                    #     --anchor_lambda ${ANCHOR_LAMBDA} \
                    #     --learning_rate ${LEARNING_RATE} \
                    #     --seed ${SEED}
                    EVAL_QUEUE+=("python -m src.evaluate \
                        --config configs/mv_2anchor_multipref.yaml \
                        --checkpoint $(ckpt_dir mv_2anchor_multipref ${MODEL_SHORT} ${SEED} ${LR_KEY} ${ANCHOR_MODEL_KEY} ${ANCHOR_LAMBDA})")
...
echo ""
echo "============================================"
echo "  All MultiPref experiments complete!"
