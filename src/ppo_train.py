"""PPO training with a distributional proxy reward model.

Migrated toward TRL 0.29.x-style PPO API.
"""
...
from src.gold_reward import load_gold_reward_scorer
from src.ppo_eval import PPOEvaluator
...
def build_run_name(
    ...
    return (
        ...
        f"_gold-{sanitize_name(gold_tag)}"
...
class PPOEvalCallback(TrainerCallback):
    def __init__(self, evaluator: PPOEvaluator, every: int):
        self.evaluator = evaluator
        self.every = every

    def _unwrap(self, model):
        """Return the underlying generate()-capable causal LM.

        Important: do NOT peel past a CausalLM into its raw backbone
        (e.g. LlamaForCausalLM -> .model -> LlamaModel), because the backbone
        no longer exposes .generate().
        """
        accel = self.evaluator.accelerator
        ...
        for attr in ("policy", "pretrained_model"):
            candidate = getattr(model, attr, None)
            if candidate is not None:
                if accel is not None:
                    try:
                        candidate = accel.unwrap_model(candidate)
                    except Exception:
                        pass
        ...
        raise AttributeError(
            f"Could not unwrap a generate()-capable policy model; got type={type(model)}"
        )

    def on_step_end(self, args, state, control, model=None, **kwargs):
        if state.global_step and state.global_step % self.every == 0 and model is not None:
            gen_model = self._unwrap(model)
            self.evaluator.evaluate(gen_model, global_step=state.global_step)
        return control
...
def main():
    ...
    if not gold_rm_model:
        raise ValueError("gold_rm_model must be set in configs/ppo.yaml or passed via --gold_rm_model.")
    ...
    gold_tag = os.path.basename(gold_rm_model.rstrip("/"))
    ...
    run_name = build_run_name(
        args.setting, args.reward_mode, policy_tag, rm_dirname, gold_tag,
        lr, kl, ppo_epochs, num_epochs, seed, train_temperature, train_top_p, rollout_batch_size,
    )
    output_dir = build_output_dir(
        policy_tag, rm_dirname, gold_tag, args.setting, args.reward_mode,
        lr, kl, ppo_epochs, num_epochs, seed, train_temperature, train_top_p, rollout_batch_size,
    )
    ...
    gold_rm, gold_tokenizer = load_gold_reward_scorer(
        gold_rm_model,
        device=str(local_device),
        dtype=torch.bfloat16,
        use_cache=False,
    ...
    eval_prompts = load_json(args.eval_prompts)
    ...
    callbacks = [
        PPOEvalCallback(evaluator, every=int(cfg["eval_every_steps"])),
        PPODebugLogCallback(),
    ...
    # Wire the real Accelerator back into the evaluator so multi-GPU eval
    # slices/gathers correctly and only the true main process logs to W&B.
    try:
        evaluator.accelerator = trainer.accelerator
    except Exception:
        pass
    ...
    if is_main:
        ...
        # Avoid Trainer.save_model() here: with this TRL experimental PPO +
        # deepspeed stack it can incorrectly treat a model config as a
        # deepspeed config and crash on .get(...).
        try:
        ...
        for attr in ("policy", "pretrained_model", "model"):
            candidate = getattr(model_to_save, attr, None)
        ...
        with open(os.path.join(output_dir, "ppo_run.json"), "w", encoding="utf-8") as f:
            json.dump(
                {
                    "run_name": run_name,
                    "setting": args.setting,
